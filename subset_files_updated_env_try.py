...
def process_all_files(audio_dir, label_dir, max_length):
    ...
    all_wav_files = [f for f in os.listdir(audio_dir) if f.endswith(".wav")]
    selected_wav_files = all_wav_files[:int(1/10 * len(all_wav_files))]
    
    for file_name in selected_wav_files:
...
directory_path = '/raid/home/kirtil/gautami/up/HF_Lung_V1-master/train/train/'
labels_directory = '/raid/home/kirtil/gautami/up/HF_Lung_V1-master/train/train/' 
None_label = 2
sample_rate = 4000
total_time = 15  
...
model.add(BatchNormalization())
model.add(Dropout(0.2))
model.add(Bidirectional(LSTM(128, return_sequences=True, dropout=0.4, recurrent_dropout=0.4)))  # Added dropout directly to LSTM
...
# 3. Increased epochs but rely on early stopping
early_stopping = EarlyStopping(monitor='val_loss', patience=5, verbose=1, mode='min', restore_best_weights=True)

# 4. Model checkpointing
from keras.callbacks import ModelCheckpoint
checkpoint_path = "/mnt/data/best_model_weights.h5"
model_checkpoint = ModelCheckpoint(checkpoint_path, monitor='val_loss', verbose=1, save_best_only=True, mode='min')

epochs = 6  
batch_size = 4
callbacks = [early_stopping, lr_scheduler, model_checkpoint]
model.fit(X_train, y_train_one_hot, batch_size=batch_size, epochs=epochs, validation_split=0.2, callbacks=callbacks, verbose=1)
model.summary(), callbacks

directory_path_for_test = '/raid/home/kirtil/gautami/up/HF_Lung_V1-master/test/'
labels_directory_test = '/raid/home/kirtil/gautami/up/HF_Lung_V1-master/test/'
