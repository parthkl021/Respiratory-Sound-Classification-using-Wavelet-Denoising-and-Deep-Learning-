...
def process_audio_and_labels(wav_file_path, label_file_path, max_length):
    ...
    fft_data_filtered[(np.abs(frequency) < 50) | (np.abs(frequency) > 1700)] = 0 # removing frequencies from a certain range
    ...
    # 1. Denoising the signal
    # 2. Extracting features from the signal (e.g., spectral features)
    # 3. ...
    ...
    for label in labels:
        ...
        if label_type in ['I', 'E']:
            ...
            if label_type == 'I':
                inhale_duration += (end_seconds - start_seconds)
            elif label_type == 'E':
                exhale_duration += (end_seconds - start_seconds)
    ...
    # Compute the ratio of inhalation duration to exhalation duration
    if exhale_duration == 0:
        inhale_exhale_ratio = float('inf')  # To handle division by zero
    else:
        inhale_exhale_ratio = inhale_duration / exhale_duration
    print(inhale_exhale_ratio)
...
model.fit(X_train, y_train_one_hot, batch_size=4, epochs=10, validation_split=0.2, callbacks=callbacks, verbose=1)
model.summary()
