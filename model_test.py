...
def process_audio_and_labels(wav_file_path, label_file_path, max_length):
    ...
    fft_data_filtered[(np.abs(frequency) < 50) | (np.abs(frequency) > 1700)] = 0 # removing frequecies from a certain range
    ...
    denoised_signal = (smoothed_envelope - np.mean(smoothed_envelope)) / np.std(smoothed_envelope)
...
# Modified code for model architecture and training process

# 1. Increased model depth and capacity
from keras.callbacks import ModelCheckpoint

model = Sequential()
model.add(Conv1D(64, kernel_size=3, activation='relu', padding='same', input_shape=(36000, 1)))
...
audio_file_path = "/home/iiitd/gautami/Parth/HF_Lung_V1-master/test/trunc_2019-08-21-11-39-26-L8_9.wav"
label_file_path = "/home/iiitd/gautami/Parth/HF_Lung_V1-master/test/trunc_2019-08-21-11-39-26-L8_8_label.txt"
signal, _ = process_audio_and_labels(audio_file_path, label_file_path, max_length=9*4000)

# Predict the labels
signal_reshaped = np.expand_dims(signal, axis=-1)
signal_reshaped = np.expand_dims(signal_reshaped, axis=0)
predictions = model.predict(signal_reshaped)
predicted_labels = np.argmax(predictions, axis=-1).squeeze()

# Trimming logic and plotting
desired_duration = 15
frame_rate = 4000  # Assuming a frame rate of 4000 Hz
num_samples = int(desired_duration * frame_rate)
trimmed_time = np.linspace(0, desired_duration, num=num_samples)

# Modify the signal variable names as per your data processing
audio_data = signal
denoised_signal = signal  # Assuming you have a denoised signal
smoothed_envelope = signal  # Assuming you have a smoothed envelope

fig, axs = plt.subplots(3, 1, figsize=(12, 6))
axs[0].plot(trimmed_time[:36000], audio_data[:num_samples])
axs[1].plot(trimmed_time[:36000], denoised_signal[:num_samples])
axs[2].plot(trimmed_time[:36000], smoothed_envelope[:num_samples])
...
# Highlighting regions based on label annotations
for label in labels:
    label_parts = label.split()
    label_type = label_parts[0]
    start_time = label_parts[1]
